...
"""
1. 该代码实现了模型结构中的一个Decoder块的代码编写
2. 首先，在forward中将翻译完的英语句子经过位置编码后，通过Masked Multi-Head Attention层
3. 再经过Linear层后，进行Add & Norm。Note: Linear层使用均匀分布初始化矩阵(random.uniform)，shape为(out, int)
4. 将Encoder的输出output_kv与上述output_q再经过一遍Multi-Head Attention层，再经过Linear层后，进行Add & Norm
5. 剩下的就是常规的feed forward和Add & Norm
"""
'''
# Part1 引入库函数
'''
