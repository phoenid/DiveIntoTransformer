...
from tf_laptop.tf_d1_5090 import de_vocab, en_vocab, de_preprocess, en_preprocess, train_dataset, PAD_IDX
# from tf_d1 import de_vocab, en_vocab, de_preprocess, en_preprocess, train_dataset, PAD_IDX
...
def collate_fn(batch):
    ...
    for enc_x, dec_x in batch:
        enc_index_batch.append(torch.tensor(enc_x, dtype=torch.long, device='cuda'))
        dec_index_batch.append(torch.tensor(dec_x, dtype=torch.long, device='cuda'))

    # 然后进行padding，因为pad_sequence只能在tensorlist用，应该batch_first表示张量以batch为第一个维度也就是(batch,seq_len)
    # pad_sequence函数是以enc_index_batch中最大句子长度为标准，使用PAD_IDX对齐
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        for pad_enc_x, pad_dec_x in dataloader:
            ...
            """
            pad_dec_x 里出现的 PAD（包括“最后一列可能是 PAD”）在 loss 里会被 ignore_index=PAD_IDX 忽略掉，不会贡献损失，也不会产生梯度更新。因此模型不会被“强迫学习预测 PAD”。
            """
            pad_enc_x = pad_enc_x.to(DEVICE)
            pad_dec_x = pad_dec_x.to(DEVICE)
